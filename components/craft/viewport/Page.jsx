'use client'

import {Box, Paper} from '@mui/material'
import {Frame, Element, useEditor} from '@craftjs/core'
import {useEffect, useState} from 'react'
import {getDeviceWidth} from '@/utils/responsive'
import {usePathname} from 'next/navigation'
import RootContainer from '../nodes/global/RootContainer'
import Container from '../nodes/global/Container'
import QuickAddBar from '../QuickAddBar'
// import demoHeaderJson from './header'
// import demoFooterJson from './footer'
// import RenderReadyHeader from './RenderReadyHeader'
// import RenderReadyFooter from './RenderReadyFooter'

export default function ViewportPage() {
  const pathname = usePathname()
  const path = pathname.split('/').slice(2).join('/') || 'home'

  const [savedData, setSavedData] = useState(undefined) // undefined = not loaded yet

  const {device, isEditing} = useEditor((state) => ({
    device: state.options.device || 'desktop',
    isEditing: state.options.isEditing !== false,
  }))

  useEffect(() => {
    if (typeof window !== 'undefined') {
      setSavedData(localStorage.getItem('craftjs-json-' + path) ?? null)
    }
  }, [])

  // wait until we've queried localStorage once
  if (savedData === undefined) return null // or a spinner

  return (
    <Box
      sx={{
        flexGrow: 1,
        bgcolor: 'grey',
        minHeight: '100vh',
      }}
    >
      <Paper
        elevation={0}
        sx={{
          width: getDeviceWidth(device),
          transition: 'width .2s ease',
          minHeight: '100%',
          bgcolor: 'background.paper',
          mx: 'auto',
          borderRadius: 0,
        }}
      >
        {/* {RenderReadyHeader(demoHeaderJson)} */}

        {savedData ? (
          <Frame json={savedData} />
        ) : (
          <Frame>
            <Element
              is={RootContainer}
              component="main"
              canvas
            >
              <Element
                is={Container}
                canvas
              />
            </Element>
          </Frame>
        )}

        {isEditing && <QuickAddBar />}

        {/* {RenderReadyFooter(demoFooterJson)} */}
      </Paper>
    </Box>
  )
}
